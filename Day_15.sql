...
*/
SELECT t1.emp_id, t1.firstname, t1.lastname, 
       CAST(SUBSTRING_INDEX(t1.s2, ',', 1) AS unsigned) AS salary, t1.department_id 
FROM (
    SELECT *, GROUP_CONCAT(salary ORDER BY salary DESC) AS s2 
    FROM Salary
